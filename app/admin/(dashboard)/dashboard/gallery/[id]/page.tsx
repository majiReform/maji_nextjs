...
import { add, edit, selectStatus, selectValue } from '@/lib/features/gallery/gallerySlice';
...
import { editGallery, singleGallery } from '@/lib/features/gallery/galleryAPI';
...
export default function Page() {
    ...
    const updateGalleryInput = async () => {
        const galleryValues = await singleGallery(params!!.id as string);
        setPictureUrl(galleryValues.response.gallery.picture);
        setOriginalPicture(galleryValues.response.gallery.picture);
    }
    ...
    return (
        <div>
            <div className='flex flex-col w-full xl:w-[800px] mx-auto py-20 gap-6'>
                ...
                <div>
                    <button className={`${buttonDisabled ? "bg-[#E6E6E6] text-[#595959]" : "bg-yellow text-[#1A1A1A]"} w-full p-3 rounded-[5px]`} disabled={buttonDisabled} onClick={submitHandler}>{status == "loading" ? (<SpinLoaderTwo />) : "Edit Image"}</button>
                </div>
