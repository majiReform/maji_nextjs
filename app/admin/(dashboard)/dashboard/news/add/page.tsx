...
import { add, selectStatus, selectValue } from '@/lib/features/news/newsSlice';
    ...
    return (
        <div>
            <div className='flex flex-col w-full xl:w-[800px] mx-auto py-20 gap-6'>
                ...
                <div className='flex flex-col'>
                    <label htmlFor="area-title">News Title</label>
                    <input type="text" className={inputClass} value={title} onChange={(e) => { setTitle(e.target.value) }} placeholder='Enter news title' />
                </div>

                <div className='flex flex-col'>
                    <label htmlFor="area-title">News Body</label>
                ...
                <div>
                    <button className={`${buttonDisabled ? "bg-[#E6E6E6] text-[#595959]" : "bg-yellow text-[#1A1A1A]"} w-full p-3 rounded-[5px]`} disabled={buttonDisabled} onClick={submitHandler}>{status == "loading" ? (<SpinLoaderTwo />) : "Upload news"}</button>
                </div>
